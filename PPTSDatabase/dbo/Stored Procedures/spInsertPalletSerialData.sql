-- ====================================================================================
-- Application 		: PPTS
-- Object Name		: spInsertPalletSerialData 
-- Author			: Namdeo
-- Created			: 20/11/2015
...
CREATE PROCEDURE [dbo].[spInsertPalletSerialData]
	...
	BEGIN TRY
		Begin transaction
			...
			declare @PartQty bigint,
					...
					@PalletBarcode varchar(100),
					@PalletSerialNo varchar(10),
					@PalletBatchCode varchar(20),
					@CustIndex varchar(10),
					@Size int,
					@cust_id as bigint,
					@custAddress as varchar(100),
					@PartDesc as varchar(30);
					set @Size=8
			...
			set @PalletBatchCode='P'+REPLICATE('0',2-LEN(RTRIM(CONVERT(varchar(2),datepart(DD,getdate()))))) + CONVERT(varchar(2),datepart(DD,getdate()))+REPLICATE('0',2-LEN(RTRIM(CONVERT(varchar(2),datepart(MM,getdate()))))) + CONVERT(varchar(2),datepart(MM,getdate()))
			...
			if( @@IDENTITY >0)
			begin
				select @custAddress=(Address_Line1+', '+Address_Line2) from MST_Customer where MST_Customer_ID=@cust_id;
				select 1 as result,TRN_PalletSerialNo_id,@PalletBarcode as PalletSrNo, 
				CONVERT(varchar(20), Created_On,113 ) as Created_On,@custAddress as CustAddress,
